# app.py - 港馬 AI 雲端預測完全體永久網頁 (內存快取徹底清洗 + PDF全動態解析版)
...
# 💥 終極最高權限防禦：開機一毫秒內強制物理轟炸清除雲端所有 Git 舊殘留快取！
try:
    st.cache_data.clear()
...
# ==========================================
# 📄 萬能物理特徵錨定器：全量提取真馬 (0硬編碼，絕無 4匹馬限制)
# ==========================================
...
if uploaded_file:
    ...
    if st.session_state.uploaded_race_data is not None and not st.session_state.uploaded_race_data.empty:
        ...
        st.sidebar.caption("舊有寫死名單已在物理上完全抹除。數據 100% 由 PDF 動態衍生。")
        
        # 💥 直接展示從 PDF 中完全動態還原出來的真實總馬匹數，如果是 4 匹馬以上，證明舊字典已死！
        ...
        # 💥 核心修正：直接過濾當前場次的「全量真實馬匹數據」，舊版 4 匹馬的幽靈字典在此處 100% 永久蒸發
        ...
        # ==========================================
        np.random.seed(4000 + selected_race)
